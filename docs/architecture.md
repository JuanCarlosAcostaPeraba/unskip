...
The urgent-attention prototype remains entirely inside `Unskip.App`. `IUrgentAttentionPreviewService` isolates the composer from WPF window creation, while `IVirtualScreenProvider` provides testable virtual-screen geometry. A borderless WPF window uses the full Windows virtual-screen rectangle, including negative monitor coordinates. Per-monitor DPI awareness is declared in the application manifest. Display-topology changes close the preview rather than risking an orphaned overlay.
